...
/* {{{ mp_read_from_mem(mp, data, data_size) */
mp_err mp_read_from_mem(mp_int *mp, uint16_t *data, uint16_t data_size)
{
    mp_err err;
    int16_t i, j;

    mp_size prec = data_size/2;
    if((err = mp_init_size(mp, prec)) != MP_OKAY)
        return err;
...
mp_err   s_mp_sub_d(mp_int *mp, mp_digit d)    /* unsigned digit subtract */
{
  ...
  w = (RADIX + (mp_word)dp[0]) - (mp_word)d;
  b = CARRYOUT(w) ? 0 : 1;
  dp[0] = ACCUM(w);

  /* Propagate borrows leftward     */
  while(b && ix < used) {
    w = (RADIX + (mp_word)dp[ix]) - (mp_word)b;
    b = CARRYOUT(w) ? 0 : 1;
    dp[ix] = ACCUM(w);
    ++ix;
  }
...
mp_err   s_mp_sub(mp_int *a, mp_int *b)        /* magnitude subtract      */
{
  ...
  for(ix = 0; ix < used; ++ix) {
    w = (RADIX + (mp_word)*pa) - w - (mp_word)*pb++;
    *pa++ = ACCUM(w);
    w = CARRYOUT(w) ? 0 : 1;
  }
...
mp_err   s_mp_mul(mp_int *a, mp_int *b)
{
  ...
  for(ix = 0; ix < ub; ++ix, ++pb) {
    ...
    for(jx = 0; jx < ua; ++jx, ++pa) {
      pt = pbt + ix + jx;
      w = (mp_word)*pb * (mp_word)*pa + k + (mp_word)*pt;
      *pt = ACCUM(w);
      k = CARRYOUT(w);
    }
...
void   s_mp_kmul(mp_digit *a, mp_digit *b, mp_digit *out, mp_size len)
{
  ...
  for(ix = 0; ix < len; ++ix, ++b) {
    ...
    for(jx = 0; jx < len; ++jx, ++pa) {
      pt = out + ix + jx;
      w = (mp_word)*b * (mp_word)*pa + k + (mp_word)*pt;
      *pt = ACCUM(w);
      k = CARRYOUT(w);
    }
...
mp_err   s_mp_sqr(mp_int *a)
{
  ...
  for(ix = 0; ix < used; ++ix, ++pa1) {
    ...
    w = (mp_word)DIGIT(&tmp, ix + ix) + ((mp_word)*pa1 * (mp_word)*pa1);
...
mp_err   s_mp_div(mp_int *a, mp_int *b)
{
  ...
  while(ix >= 0) {
    ...
    if(q <= DIGIT(b, USED(b) - 1) && USED(&rem) > 1)
      q = (q << DIGIT_BIT) | (mp_word)DIGIT(&rem, USED(&rem) - 2);
